...
setwd('F:\\ResearchProject\\ARI_BioMass\\BD_2017-18_DHS_12012021_346_170459\\BDKR7RDT')
...
logit1 <- (svyglm(ari~v161+roof+age+b4, family=quasibinomial(link=logit), data=kr,design=design1, na.action = na.omit))
summary(logit1) #sig
vif(logit1)
...
ARI <- read.csv("F:\\ResearchProject\\ARI_BioMass\\ARI.csv")
...
ARI <- read.csv("F:\\ResearchProject\\ARI_BioMass\\BF.csv")
...
df <- data.frame(value = c(79.6, 20.1),
